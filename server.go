...
func commandToBytes(commad string) []byte {
	var bytes [commandLength]byte
	for i, c := range commad {
		bytes[i] = byte(c)
	}
	return bytes[:]
}
...
func handleAddr(request []byte) {
	...
	fmt.Printf("There are %d know node now\n", len(knownNodes))
	requestBlocks()
}
...
func handleInv(request []byte, bc *Blockchain) {
	...
	fmt.Printf("Receive invetory with %d %d\n", len(payload.Items), payload.Type)
